...
func main() {
	...
	h, w, k := ReadInt3()
	board := NewBoard(h, w, func(r rune) bool {
		return r == '#'
	})
	...
	for y := 0; y < h; y++ {
		for x := 0; x < w; x++ {
			routes += dfs(board, Position{x, y}, k)
		}
	}
...
func dfs(board Board[bool], pos Position, k int) int {
	if board.In(pos) {
		return 0
	}
	if board.IsVisited(pos) {
		return 0
	}
	if board.At(pos).value {
		return 0
	}
	if k == 0 {
		return 1
	...
	board.Visit(pos)
	routes := 0
	for _, p := range board.Neighbors4(pos) {
		routes += dfs(board, p, k-1)
	}
	board.Leave(pos)
