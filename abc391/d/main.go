...
func main() {
	...
	for i := 0; i < n; i++ {
		x, y := ReadInt2()
		blocks[i].a = i + 1
		blocks[i].p = Position{x: x, y: y}
		blocks[i].t = 1000_000_000 + 1
	}
	...
	for i, b := range blocks {
		row[b.p.x] = append(row[b.p.x], &blocks[i])
	}
	for i := range row {
		sort.Slice(row[i], func(j, k int) bool {
			return row[i][j].p.y < row[i][k].p.y
		})
	}
