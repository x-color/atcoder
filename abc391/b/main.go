...
func main() {
	...
	n, m := ReadInt2()
	s := NewBoard(n, n, func(r rune) Square[rune] {
		return Square[rune]{value: r}
	})
	t := NewBoard(m, m, func(r rune) Square[rune] {
		return Square[rune]{value: r}
	})
	...
	for y := 0; y < n-m+1; y++ {
		for x := 0; x < n-m+1; x++ {
			if s.At(Position{x: x, y: y}).value == t.At(Position{0, 0}).value {
				ok := true
				for ty := 0; ty < m; ty++ {
					for tx := 0; tx < m; tx++ {
						if s.At(Position{x: x + tx, y: y + ty}).value != t.At(Position{tx, ty}).value {
							ok = false
							break
						}
					}
	...
	fmt.Fprintln(out, ansY+1, ansX+1)
}
