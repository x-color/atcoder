...
func main() {
	...
	h, w := ReadInt2()
	y, x := ReadInt2()
	board := NewBoard(h, w, func(r rune) Square[rune] {
		return Square[rune]{value: r}
	})
	t := ReadString()

	c := 0
	p := Position{x: x - 1, y: y - 1}
	for _, r := range t {
		dif := Position{x: 0, y: 0}
		switch r {
		case 'U':
			dif.y = -1
		case 'D':
		...
		next := Position{x: p.x + dif.x, y: p.y + dif.y}
		if board.In(next) && board.At(next).value.value != '#' {
			p = next
			if board.At(p).value.value == '@' && !board.IsVisited(p) {
				board.Visit(p)
				c++
			}
		}
	...
	fmt.Fprintln(out, p.y+1, p.x+1, c)
}
